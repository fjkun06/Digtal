...
import MobileActiveNavLink, {
  ActiveNavLink
} from "../reusables/ReusableNavLink";
import ExpandLessOutlinedIcon from "@mui/icons-material/ExpandLessOutlined";
import {
  flagItems,
  navbarItems,
  selectDropdownItems
} from "../../layouts/navbar/navbar_config";
import { icons } from "../../layouts/footer/footer_config";
import { useTranslation } from "react-i18next";
import Search from "src/assets/svg/Search";

const MobileNavbarBody = ({
  language,
  showMobileMenu,
  toggleMobileSelect,
  mobileSelectState,
  ...
  return (
    <Grid
      item
      container
      className="main-nav-sub--mobile-body"
      sx={{ display: !showMobileMenu ? "none !important" : "grid" }}
    >
      ...
      <div className="mobile-body--section menu">
        <div className="menu-sub">
          {navbarItems.slice(0, 2).map((item, index) => (
            <Grid item key={index} id={item.id}>
              <ActiveNavLink
                ...
                text={t(item.index)}
                toggleMobile={setShowMobileMenu}
              />
            </Grid>
          ))}
          ...
          <Grid sx={{ width: "fit-content" }} id="flags">
            {flagItems.map(flag => (
              <div key={flag.id}>
                <img
                  src={flag.src}
