...
export const CarouselControl = ({ autoplay, match870, setIndex, setAutoplay, currentIndex }) => {
  ...
  return (
    <div className="carousel">
      {[0, 1, 2, 3].map((elt, index) => (match480 ? <button key={elt} onClick={() => setIndex(elt)} className="carousel-button" style={{ backgroundColor: index !== currentIndex ? "" : "rgba(0, 208, 255, 0.302)" }}></button> : <button key={elt} onClick={() => setIndex(elt)} className="carousel-button" style={{ height: index === currentIndex ? "8px" : "" }}></button>))}
